...
export class ClientesFormComponent implements OnInit {
  ...
  titulo = 'Formulario crear cliente'
  ...
  crear(): void{
    this.clienteServicio.crear(this.cliente).subscribe(cliente => {
      swal.fire('Cliente Creado', `El cliente ${cliente.name} ${cliente.lastname} fue creado con éxito`, 'success');
  ...
  editar(): void{
    this.clienteServicio.editar(this.cliente).subscribe(cliente => {
      swal.fire('Cliente Creado', `El cliente ${cliente.name} ${cliente.lastname} fue actualizado con éxito`, 'success');
      this.router.navigate(['/clientes']);
    }, err => {
      if(err.status == 400){
        this.error = err.error;
